...
(
    ...
    TOTP_MENU,
    ADD_WAITING, ADD_MANUAL_NAME, ADD_MANUAL_SECRET, ADD_MANUAL_TYPE, ADD_MANUAL_HOTP_COUNTER,
...
BOT_USERNAME        = os.environ.get("BOT_USERNAME", "TotpNafiBot")
    ...
    with get_db() as c:
        ...
        for col, defval in [("tg_name", "''"), ("timezone", "'UTC'"), ("last_backup_reminder", "0")]:
            try:
                ...
                pass
        for col in ["note", "otp_type", "hotp_counter"]:
            try:
                c.execute(f"ALTER TABLE totp_accounts ADD COLUMN {col} TEXT DEFAULT ''")
            except Exception:
                pass
...
def steam_totp_now(secret: str):
    ...
    code_str = ""
    for _ in range(5):
        code_str += alphabet[code_int % len(alphabet)]
...
def parse_otpauth(uri: str):
    try:
        ...
        if not ok:
            return None
        otp_type = params.get("type", ["totp"])[0].lower()
        counter = int(params.get("counter", [0])[0]) if otp_type == "hotp" else 0
        return {"name": name, "issuer": issuer, "secret": c, "type": otp_type, "counter": counter}
    except Exception:
        return None
...
def record_login_attempt(tid: int) -> bool:
    now = int(time.time())
    with get_db() as c:
...
def reset_login_attempts(tid: int):
    with get_db() as c:
        c.execute("DELETE FROM login_attempts WHERE telegram_id=?", (tid,))
        c.commit()

def is_login_frozen(tid: int) -> bool:
    with get_db() as c:
        row = c.execute("SELECT frozen_until FROM login_attempts WHERE telegram_id=?", (tid,)).fetchone()
        return bool(row and row["frozen_until"] > int(time.time()))

def get_login_freeze_remaining(tid: int) -> int:
    with get_db() as c:
        row = c.execute("SELECT frozen_until FROM login_attempts WHERE telegram_id=?", (tid,)).fetchone()
...
# ── Backup Reminder ─────────────────────────────────────────
def should_send_backup_reminder(telegram_id: int) -> bool:
    with get_db() as c:
        row = c.execute("SELECT last_backup_reminder FROM users WHERE telegram_id=?", (telegram_id,)).fetchone()
        if not row:
            return False
...
def update_backup_reminder(telegram_id: int):
    with get_db() as c:
        c.execute("UPDATE users SET last_backup_reminder=? WHERE telegram_id=?", (int(time.time()), telegram_id))
        c.commit()
...
def kb_main():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🔍 Search TOTP", callback_data="search_totp"),
...
def build_import_conflict_kb():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🔄 Skip duplicates", callback_data="import_skip")],
        [InlineKeyboardButton("✏️ Overwrite duplicates", callback_data="import_overwrite")],
        [InlineKeyboardButton("➕ Merge (keep both)", callback_data="import_merge")],
        [InlineKeyboardButton("❌ Cancel", callback_data="cancel_to_menu")],
    ])

def build_hotp_next_kb(acc_id: int):
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🔄 Next Code", callback_data=f"hotp_next_{acc_id}")],
        [InlineKeyboardButton("🏠 Main Menu", callback_data="main_menu")],
    ])
        ...
        if u:
            ...
            if should_send_backup_reminder(uid):
                await update.message.reply_text(
                    "📦 *Backup Reminder*\n\n"
                    "It's been over a week since your last vault backup.\n"
                    "Use /export to create an encrypted backup of your TOTP codes.\n"
                    "_Keep your backup file safe!_",
                    parse_mode="MarkdownV2",
                )
                update_backup_reminder(uid)
            await update.message.reply_text(
...
async def login_auto(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await q.answer()
    uid = update.effective_user.id
    if is_login_frozen(uid):
        remaining = get_login_freeze_remaining(uid)
        h, m = remaining // 3600, (remaining % 3600) // 60
        await q.edit_message_text(
            f"⚠️ *Too many failed login attempts\\.* Account frozen for *{h}h {m}m*\\.",
            parse_mode="MarkdownV2",
            reply_markup=kb_auth(),
        )
        return AUTH_MENU
...
async def login_id_input(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not u:
        ...
        return LOGIN_ID_INPUT
    tid = u["telegram_id"]
    if is_login_frozen(tid):
        remaining = get_login_freeze_remaining(tid)
        h, m = remaining // 3600, (remaining % 3600) // 60
        await update.message.reply_text(
            f"⚠️ *Account frozen for *{h}h {m}m* due to failed attempts\\.",
            parse_mode="MarkdownV2",
            reply_markup=kb_auth(),
        )
        return AUTH_MENU
...
async def login_pw(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not hmac.compare_digest(hash_pw(pw, bytes(u["pw_salt"])), bytes(u["password_hash"])):
        ...
        if frozen:
            ...
            ctx.user_data.pop("login_vid", None)
            return AUTH_MENU
        else:
            with get_db() as c:
                row = c.execute("SELECT attempts FROM login_attempts WHERE telegram_id=?", (u["telegram_id"],)).fetchone()
                attempts = row["attempts"] if row else 0
                left = max(0, MAX_LOGIN_ATTEMPTS - attempts)
            await update.message.reply_text(
                f"❌ Wrong password\\. {left} attempt\\(s\\) remaining before freeze\\.",
                parse_mode="MarkdownV2",
                reply_markup=kb_cancel(),
            )
            return LOGIN_PASSWORD
...
async def reset_new_pw_confirm(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    with get_db() as c:
        rows = c.execute(
            "SELECT id, secret_enc, salt, iv, sk_enc, sk_salt, sk_iv, otp_type, hotp_counter, note "
...
async def settings_reset_pw_confirm(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    with get_db() as c:
        rows = c.execute(
            "SELECT id, secret_enc, salt, iv, otp_type, hotp_counter, note FROM totp_accounts WHERE vault_id=?", (vault,)
        ).fetchall()
        for row in rows:
            try:
...
async def _do_save_totp(update, vault, data, pw):
    secret = data["secret"]
    otp_type = data.get("type", "totp")
    hotp_counter = data.get("counter", 0)
    note = data.get("note", "")
    ...
    if otp_type == "totp":
        ...
        code = steam_totp_now(secret)
        remain = 30 - (int(time.time()) % 30)
        next_code = ""
    else:  # hotp
        code = hotp_now(secret, hotp_counter)
        remain = 0
        next_code = hotp_now(secret, hotp_counter + 1)
    issuer_line = f"\n_{em(data['issuer'])}_" if data.get("issuer") else ""
    note_line = f"\n📝 *Note:* {em(note[:10])}" if note else ""
    text = f"✅ *{em(data['name'])}* added\\!{issuer_line}{note_line}\n\n"
    if otp_type == "hotp":
        ...
        text += f"🔢 `{code[:3]} {code[3:]}`\n"
        if otp_type == "totp":
            text += f"⏱ {bar(remain)} {remain}s\n"
        else:
            text += f"⏱ {bar(remain)} {remain}s (Steam style)\n"
        text += f"🔒 _Encrypted with AES\\-256\\-GCM \\+ Secure Key_"
    ...
    if ok and len(cleaned) >= 8:
        try:
            ...
            await update.message.reply_text(
                ...
                parse_mode="MarkdownV2",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("⏱ TOTP (30 sec)", callback_data="add_type_totp")],
                    [InlineKeyboardButton("🔥 HOTP (counter)", callback_data="add_type_hotp")],
                    [InlineKeyboardButton("🎮 Steam TOTP", callback_data="add_type_steam")],
...
async def add_type_choice(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if otp_type == "hotp":
        await q.edit_message_text(
            "Enter initial counter (integer, default 0):",
            parse_mode="MarkdownV2",
            reply_markup=kb_cancel(),
        )
...
async def add_manual_hotp_counter(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    text = update.message.text.strip()
    try:
        await update.message.delete()
    except Exception:
    ...
    try:
        ...
        await update.message.reply_text(
            "⚠️ Invalid counter. Enter a non-negative integer:",
            parse_mode="MarkdownV2",
            reply_markup=kb_cancel(),
        )
        return ADD_MANUAL_HOTP_COUNTER
    ctx.user_data["pending_counter"] = counter
    await update.message.reply_text(
        "Enter *account name:*",
        parse_mode="MarkdownV2",
        reply_markup=kb_cancel(),
...
async def handle_manual_name(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    otp_type = ctx.user_data.pop("pending_type", "totp")
    hotp_counter = ctx.user_data.pop("pending_counter", 0)
    if preloaded:
        uid = update.effective_user.id
        vault = get_session(uid)
        pw = ctx.user_data.get("password")
        return await _do_save_totp(update, vault, {"name": name, "issuer": "", "secret": preloaded, "type": otp_type, "counter": hotp_counter}, pw)
    ctx.user_data["pending_name"] = name
    ctx.user_data["pending_type"] = otp_type
    ctx.user_data["pending_counter"] = hotp_counter
    await update.message.reply_text(
        f"✅ Name: *{em(name)}*\n\n"
...
async def handle_manual_secret(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(
            "⚠️ *Secret key failed test\\.* Try again:",
            parse_mode="MarkdownV2",
            reply_markup=kb_cancel(),
        )
        return ADD_MANUAL_SECRET
...
async def list_totp(update: Update, ctx: ContextTypes.DEFAULT_TYPE, page: int = 0):
    ...
    for row in page_rows:
        try:
            ...
            if row["otp_type"] == "totp":
                code, remain = totp_now(secret)
                next_code = ""
                type_str = "TOTP"
            elif row["otp_type"] == "steam":
                code = steam_totp_now(secret)
                remain = 30 - (int(time.time()) % 30)
                next_code = ""
                type_str = "Steam"
            else:  # hotp
                code = hotp_now(secret, row["hotp_counter"])
                remain = 0
                next_code = hotp_now(secret, row["hotp_counter"] + 1)
                type_str = "HOTP"
            issuer_line = f"_{em(row['issuer'])}_  " if row["issuer"] else ""
            note_line = f" 📝{em(row['note'][:10])}" if row["note"] else ""
            line = f"{issuer_line}*{em(row['name'])}*{note_line} [{type_str}]\n"
            line += f"`{code[:3]} {code[3:]}`"
            if row["otp_type"] == "hotp":
                line += f"\n⏩ Next: `{next_code[:3]} {next_code[3:]}`"
            else:
                line += f" {bar(remain)} {remain}s"
            lines.append(line)
        except Exception as e:
...
async def search_totp_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await q.edit_message_text(
        ...
        "Use `#` prefix to search in names and notes (e.g., `#github`).\n\n"
...
async def search_totp_query(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
...
async def edit_pick(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    with get_db() as c:
        row = c.execute(
            "SELECT name, note FROM totp_accounts WHERE id=? AND vault_id=?", (acc_id, vault)
        ).fetchone()
    if not row:
    ...
    ctx.user_data["edit_note"] = row["note"] or ""
    await q.edit_message_text(
...
async def edit_action(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "rename":
        ...
        current_note = ctx.user_data.get("edit_note", "")
        await q.edit_message_text(
            f"📝 *Edit Note* (max 10 chars)\n\nCurrent: `{em(current_note)}`\n\nSend new note (or 0 to clear):",
            parse_mode="MarkdownV2",
            reply_markup=kb_cancel(),
        )
...
async def edit_note_input(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if len(note) > 10:
        await update.message.reply_text(
            "⚠️ Note too long. Maximum 10 characters. Try again:",
            parse_mode="MarkdownV2",
            reply_markup=kb_cancel(),
        )
        return EDIT_NOTE_INPUT
    ...
    if not acc_id:
        await update.message.reply_text("Session error.", parse_mode="MarkdownV2", reply_markup=kb_main())
        return TOTP_MENU
    with get_db() as c:
        c.execute("UPDATE totp_accounts SET note=? WHERE id=? AND vault_id=?", (note[:10], acc_id, vault))
        c.commit()
    await update.message.reply_text(f"✅ Note updated to: `{em(note[:10])}`", parse_mode="MarkdownV2", reply_markup=kb_main())
    ctx.user_data.pop("edit_id", None)
    ctx.user_data.pop("edit_name", None)
    ctx.user_data.pop("edit_note", None)
    return TOTP_MENU
...
# ── HOTP Next Code ─────────────────────────────────────────
async def hotp_next(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    await q.answer()
    try:
    ...
    with get_db() as c:
        row = c.execute(
            "SELECT secret_enc, salt, iv, hotp_counter, name FROM totp_accounts WHERE id=? AND vault_id=? AND otp_type='hotp'",
            (acc_id, vault)
        ).fetchone()
        if not row:
            await q.answer("Account not found or not HOTP.", show_alert=True)
            return TOTP_MENU
        secret = decrypt(row["secret_enc"], row["salt"], row["iv"], pw, vault)
        new_counter = row["hotp_counter"] + 1
        c.execute("UPDATE totp_accounts SET hotp_counter=? WHERE id=?", (new_counter, acc_id))
        c.commit()
    code = hotp_now(secret, new_counter)
    next_code = hotp_now(secret, new_counter + 1)
    text = f"*{em(row['name'])}* (HOTP)\n\n"
    text += f"🔢 Current code: `{code[:3]} {code[3:]}`\n"
    text += f"⏩ Next code: `{next_code[:3]} {next_code[3:]}`"
    await q.edit_message_text(text, parse_mode="MarkdownV2", reply_markup=build_hotp_next_kb(acc_id))
    return TOTP_MENU
...
async def export_pw2_input(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    with get_db() as c:
        rows = c.execute(
            "SELECT name, issuer, secret_enc, salt, iv, otp_type, hotp_counter, note FROM totp_accounts WHERE vault_id=?", (vault,)
        ).fetchall()
    if not rows:
        await update.message.reply_text(
    ...
    for row in rows:
        try:
            secret = decrypt(row["secret_enc"], row["salt"], row["iv"], pw, vault)
            entries.append({
                "name": row["name"], "issuer": row["issuer"] or "", "secret": secret,
                "otp_type": row["otp_type"], "hotp_counter": row["hotp_counter"], "note": row["note"]
            })
        except Exception as e:
...
# ── IMPORT VAULT with overwrite option ─────────────────────
...
async def import_pw_input(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"✅ File loaded: {len(accounts)} account(s) found.\n\n"
        "Choose conflict resolution for duplicate names:",
        parse_mode="MarkdownV2",
        reply_markup=build_import_conflict_kb(),
    )
...
async def import_conflict_choice(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    with get_db() as c:
        ...
        for acc in accounts:
            name = acc["name"]
            if choice == "skip" and name in existing_names:
                skipped += 1
                continue
            ...
            try:
                ...
                if otp_type == "totp":
                    ...
                    steam_totp_now(secret)
                else:
                    hotp_now(secret, acc.get("hotp_counter", 0))
                ...
                c.execute(
                    ...
                    (vault, name, acc.get("issuer", ""), ct, s, iv, sk_ct, sk_s, sk_iv,
                     otp_type, acc.get("hotp_counter", 0), acc.get("note", "")[:10]),
                )
                imported += 1
            except Exception as e:
...
async def global_add_cancel(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    ctx.user_data.pop("_global_add", None)
    ctx.user_data.pop("pending_type", None)
    ctx.user_data.pop("pending_counter", None)
...
async def cancel_to_menu(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await q.answer()
    for k in [
        "pending_name", "signup_pw", "new_pw", "edit_id", "edit_name", "edit_note",
        "pending_secret", "_global_add", "reset_vid", "sreset_pw",
        "import_payload", "delete_vault", "delete_owner", "import_accounts",
        "reset_secure_key", "reset_sk_skipped", "reset_new_pw", "reset_otp_verified",
        "share_selected", "share_rows", "pending_type", "pending_counter", "search_query"
    ]:
        ctx.user_data.pop(k, None)
    uid = update.effective_user.id
    vault = get_session(uid)
    ...
    conv = ConversationHandler(
        ...
        states={
            ...
            TOTP_MENU: [
                ...
                CallbackQueryHandler(edit_action,          pattern=r"^edit_action_(rename|note|delete|showsecret)$"),
                ...
                CallbackQueryHandler(hotp_next,        pattern=r"^hotp_next_\d+$"),
            ],
            ADD_WAITING: [
            ...
            ADD_MANUAL_HOTP_COUNTER: [
                MessageHandler(private & filters.TEXT & ~filters.COMMAND, add_manual_hotp_counter),
                CallbackQueryHandler(cancel_to_menu, pattern="^cancel_to_menu$"),
            ],
            EDIT_PICK: [
                CallbackQueryHandler(edit_pick,    pattern=r"^editpick_\d+$"),
            ...
            EDIT_NOTE_INPUT: [
                MessageHandler(private & filters.TEXT & ~filters.COMMAND, edit_note_input),
                CallbackQueryHandler(cancel_to_menu, pattern="^cancel_to_menu$"),
            ],
            SHOW_SECRET_PW: [
            ...
            IMPORT_CONFLICT_CHOICE: [
                CallbackQueryHandler(import_conflict_choice, pattern="^import_(skip|overwrite|merge)$"),
                CallbackQueryHandler(cancel_to_menu, pattern="^cancel_to_menu$"),
            ],
            TZ_INPUT: [
            ...
            SEARCH_QUERY: [
                MessageHandler(private & filters.TEXT & ~filters.COMMAND, search_totp_query),
                CallbackQueryHandler(cancel_to_menu, pattern="^cancel_to_menu$"),
            ],
            HOTP_NEXT: [
                CallbackQueryHandler(hotp_next, pattern=r"^hotp_next_\d+$"),
